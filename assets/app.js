...
(function () {
  ...
  function menuKeydown(e) {
    ...
    if (e.key === 'Tab') {
      const f = $$('a, button', overlay).filter(el => el.offsetParent !== null);
      if (!f.length) return;
  ...
  function setupIlloAnim() {
    ...
    function classify(svg) {
      const guides = [], draws = [], fills = [];
      $$('path,line,circle,rect,polyline,polygon,text,ellipse', svg).forEach((el) => {
        const fill = el.getAttribute('fill');
        const hasFill = fill && fill !== 'none';
        const stroke = el.getAttribute('stroke');
        const hasStroke = stroke && stroke !== 'none';
        if (el.tagName === 'text' || (hasFill && !hasStroke)) { fills.push(el); return; }
        if (!hasStroke) { fills.push(el); return; }
        if (el.getAttribute('stroke-dasharray')) { guides.push(el); return; }
        draws.push(el);
      });
      return { guides, draws, fills };
    }

    function prep(svg) {
      const g = classify(svg);
      g.guides.forEach((el) => { el.style.opacity = '0'; });
      g.draws.forEach((el) => {
        let len = 0;
        ...
        } else { el.style.opacity = '0'; el.dataset.illoFade = '1'; }
      });
      g.fills.forEach((el) => {
        el.style.opacity = '0';
    ...
    function play(svg) {
      if (svg.dataset.illoPlayed) return;
      svg.dataset.illoPlayed = '1';
      const g = svg._illoGroups || classify(svg);
      const ease = 'cubic-bezier(.33,.7,.3,1)';
      g.guides.forEach((el, i) => {
        el.style.transition = `opacity .5s ease ${(i * 0.035).toFixed(3)}s`;
        el.style.opacity = '1';
      });
      const drawBase = 0.12 + g.guides.length * 0.035;
      g.draws.forEach((el, i) => {
        const d = (drawBase + i * 0.055).toFixed(3);
        if (el.dataset.illoFade) {
          el.style.transition = `opacity .5s ease ${d}s`;
          el.style.opacity = '1';
        } else {
          el.style.transition = `stroke-dashoffset .72s ${ease} ${d}s`;
          el.style.strokeDashoffset = '0';
        }
      });
      const fillBase = drawBase + g.draws.length * 0.055 + 0.05;
      g.fills.forEach((el, i) => {
        const d = (fillBase + i * 0.06).toFixed(3);
        el.style.transition = `opacity .42s ease ${d}s, transform .55s cubic-bezier(.2,1.5,.4,1) ${d}s`;
        el.style.opacity = '1';
  ...
  if (qc) {
    ...
    if (viewport) { viewport.setAttribute('aria-live', 'polite'); viewport.setAttribute('aria-atomic', 'true'); }
    ...
    qc.addEventListener('focusin', () => { if (timer) clearInterval(timer); });
    qc.addEventListener('focusout', restart);
  ...
  if (canvas && !reduce) {
    ...
    function draw() {
      ...
      if (motesVisible) motesRaf = requestAnimationFrame(draw);
    }
    let motesRaf = null, motesVisible = true;
    function startMotes() { if (!motesRaf && motesVisible) motesRaf = requestAnimationFrame(draw); }
    const heroEl = $('.hero');
    if (heroEl && 'IntersectionObserver' in window) {
      new IntersectionObserver((ents) => {
        motesVisible = ents[0].isIntersecting;
        if (motesVisible) startMotes();
